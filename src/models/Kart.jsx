...
export function Kart({
  ...
  function getGroundPosition(wheelBase, wheel, offset = 0, wheelIndex, delta) {
    ...
    // vertical wall faces and tall barrier tops (all named "ground") — those
    // must never set wheel height or the kart teleports/flips.
    const targets =
  ...
  // Max visual tilt of the kart body — never let it flip over.
  function moveAndRotateKart(wheelPositions, delta) {
    ...
    let pitch = (c.y + d.y - (a.y + b.y)) * 0.5;
    let roll = (b.y - a.y + d.y - c.y) * 0.5;
            ...
            <group ref={starsGroupRef} position={[0, 1.35, -0.1]} visible={false}>
              {[0, 1, 2, 3, 4, 5].map((i) => {
                ...
                return (
                  <sprite
                    key={i}
                    position={[Math.cos(a) * 0.9, (i % 2) * 0.25, Math.sin(a) * 0.9]}
                    scale={[0.45, 0.45, 1]}
                  >
                    <spriteMaterial
                      map={starsTex}
